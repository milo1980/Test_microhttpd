...
//	FILE *fp;
	...
	{
		strncpy(con_info->terminal.transactionsSupported[con_info->transactionIndex++],data,64);
	}
	else
	{
//		return MHD_NO;
...
static void request_completed(void *cls, struct MHD_Connection *connection,
	...
	if (con_info->connectiontype == POST) {
		...
		memcpy(&terminalList[nr_of_terminals],&con_info->terminal,sizeof(terminalinfo_t));
		...
		if (0 == url) {
			...
			if (0 == strcmp(url, "/")) {
				...
				if (0 == strcmp(url, "create_terminals")) {
					/*create and send responses*/
					snprintf(pageBuffer, sizeof(pageBuffer),
							createTerminalPage, itoa(nr_of_terminals,tempString,10));
